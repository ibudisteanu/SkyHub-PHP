...
$objTwitterOAuth2 = new Twitter_OAuth2();
...
if (!empty($_GET['oauth_token'])) {
    ...
    $objTwitterOAuth2->arrResult = json_decode($twitterService->request('account/verify_credentials.json'),true);
    $objTwitterOAuth2->bSuccess=true;
    ...
    $url = $twitterService->getAuthorizationUri(array('oauth_token' => $token->getRequestToken(),"oauth_callback"=>"url"));
    //$url = $twitterService->getAuthorizationUri(array('oauth_token' => $token->getRequestToken(),"oauth_callback"=>"http://myskyhub.ddns.net/signup/twitter"));
    header('Location: ' . $url);
} else {
